...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='competition_data.csv',
                 target='INSP', scale=True, timeenc=0, freq='h'):
    ...
    def __read_data__(self):
        ...
        '''
        df_raw.columns: ['STD_DT', ...(other features), target feature]
        '''
        cols = ['STD_DT', 'MELT_TEMP', 'MOTORSPEED', 'MELT_WEIGHT', 'INSP']
        cols.remove(self.target)
        cols.remove('STD_DT')
        df_raw = df_raw[['STD_DT'] + cols + [self.target]]
        # print(cols)
        ...
        if self.timeenc == 0:
            ...
            data_stamp = time_features(pd.to_datetime(df_stamp['STD_DT'].values), freq=self.freq)
